...
End-to-end NLP pipeline that classifies SMS messages as **spam** or **ham** using the UCI SMS Spam Collection dataset (5,572 messages). Four classifiers are trained, evaluated, and compared — the best model is exported for inference.
...
**Random Forest** is selected as the best model. It ties near-top accuracy while achieving **zero false positives** — no legitimate message is ever mislabeled as spam.
...
| File | Description |
|------|-------------|
| `class_distribution.png` | Class imbalance bar chart + message length distribution |
...
[UCI SMS Spam Collection](https://archive.ics.uci.edu/ml/datasets/SMS+Spam+Collection) — 5,572 SMS messages: **4,825 ham (86.6%)** / **747 spam (13.4%)**.
